...
fastify.post('/process-urls', async (req, reply) => {
  try {
    ...
    for (let i = 0; i < vectors.length; i += 10) {
      await ns.upsert(vectors.slice(i, i + 10).map(({id, values, metadata}) => ({id, values, metadata})))
    }
...
fastify.post('/query', async (req, reply) => {
  try {
    ...
    const retrievedChunks = await ns.query({vector: queryEmbeddings, topK: 7, includeMetadata: true})
